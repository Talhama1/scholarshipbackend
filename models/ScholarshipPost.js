...
const ScholarshipForm = ({ onScholarshipAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://192.168.1.3:3000/123/scholarship', {
        method: 'POST',
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.1.3:3000/123/scholarship/by-name/${encodeURIComponent(deleteName)}`, {
        method: 'DELETE',
      });
